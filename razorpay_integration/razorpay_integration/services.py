...
from razorpay_integration.utils import (
    get_settings,
    is_doctype_enabled,
...
def build_reference_context(reference_doctype: str, reference_name: str) -> dict:
    ...
    data = {
        ...
        'lead': reference_name if reference_doctype == 'Lead' else None,
...
def create_payment_intent_doc(reference_doctype: str, reference_name: str, amount, request_type=None, request_channel=None, notes=None):
    settings = get_settings()
    if not settings.provider_enabled:
        frappe.throw('Payment provider is disabled in Razorpay Integration Settings')
    if not is_doctype_enabled(reference_doctype):
        frappe.throw(f'Payment requests are disabled for {reference_doctype}')
    if request_channel != 'POS' and not settings.enable_payment_links:
        frappe.throw('Payment link generation is disabled in Razorpay Integration Settings')
    if request_channel == 'POS' and not getattr(settings, 'enable_pos_payments', 0):
